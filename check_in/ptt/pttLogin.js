...
!(async () => {
  try {
    const response = await $.http.get("https://postman-echo.com/get?foo1=bar1&foo2=bar2");
    $.log(`HTTP GET 響應：${JSON.stringify(response)}`);
  } catch (error) {
    $.error(`HTTP GET 錯誤：${error}`);
  }
})();
